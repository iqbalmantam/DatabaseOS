...
SPREADSHEET_URL = "https://docs.google.com/spreadsheets/d/1HFv_6BHGDhQmCNFbXaFr_Q_8jRbJILEQUwF2aWh4oWU/edit"


def get_spreadsheet_url():
    """Mengambil URL dari secrets jika ada, jika tidak pakai SPREADSHEET_URL."""
    try:
        return st.secrets["connections"]["gsheets"].get("spreadsheet", SPREADSHEET_URL)
    except Exception:
        return SPREADSHEET_URL


def get_connection():
    # Inisialisasi koneksi standar tanpa mengirimkan kwarg spreadsheet
...
def load_master_data():
    ...
    try:
        df = conn.read(spreadsheet=url, worksheet="Master_Karyawan", ttl=0)
...
def load_snapshot_data():
    ...
    url = get_spreadsheet_url()
    try:
        df_snap = conn.read(spreadsheet=url, worksheet="Snapshot_Bulanan", ttl=0)
...
def load_absensi_data():
    ...
    url = get_spreadsheet_url()
    try:
        df_absen = conn.read(spreadsheet=url, worksheet="Absensi_Karyawan", ttl=0)
...
def load_manpower_cost_data(headers):
    ...
    df_mc = None
    try:
        df_mc = conn.read(spreadsheet=url, worksheet="Manpower_Cost", ttl=0)
    except Exception:
        try:
            df_mc = conn.read(spreadsheet=url, ttl=0)
        except Exception:
            df_mc = None
