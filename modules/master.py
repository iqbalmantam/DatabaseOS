...
def render_page(is_admin):
    ...
    selected_dash_period = st.selectbox("📅 Pilih Periode Dashboard:", options=available_periods, index=0)
    ...
    df_active_filtered, df_resign_filtered, df_pie_chart = filter_status_for_period(df_ana, active_period_str, selected_dash_period)

    total_aktif = len(df_active_filtered)
    total_resign = len(df_resign_filtered)

    col_m1, col_m2, col_m3 = st.columns(3)
    with col_m1:
        st.metric(label=f"Karyawan Aktif ({active_period_str})", value=total_aktif)
    with col_m2:
        st.metric(label=f"Karyawan Resign ({active_period_str})", value=total_resign)
    with col_m3:
            ...
            with st.form("add_employee_form", clear_on_submit=True):
                ...
                if submit_btn:
                    ...
                    existing_ids = (
                        ...
                        if "ID" in st.session_state.employees.columns else []
                    )
            ...
            if import_type == "File CSV":
                ...
                if uploaded_file is not None and st.button("Mulai Import File"):
                    try:
                        ...
                        df_import_filtered = df_import[~df_import["ID"].astype(str).str.strip().str.upper().isin(existing_ids)]
            ...
            if st.button(f"🔒 Kunci Data {selected_periode}"):
                try:
                    ...
                    cols_order = [
                        ...
                        "Tanggal Bergabung", "Akhir Kontrak", "Tanggal Resign", "Site",
                        "Status", "Terakhir Diperbarui", "Tanggal Snapshot"
                    ]
            ...
            with tab_overview:
                ...
                with c1:
                    if "Status" in df_pie_chart.columns and not df_pie_chart.empty:
                        fig_status = plot_pie_status(df_pie_chart, active_period_str)
                        st.plotly_chart(fig_status, use_container_width=True)
                    else:
                        st.info("Tidak ada data status untuk ditampilkan.")
                with c2:
                    if "Posisi" in df_active_filtered.columns:
                        top_roles = df_active_filtered["Posisi"].value_counts().head(10).reset_index()
            ...
            with tab_cost:
                ...
                with c3:
                    if "Cost Center" in df_active_filtered.columns:
                        ...
                        df_cc_clean["Cost Center Clean"] = df_cc_clean["Cost Center Clean"].replace({
                            "Vinfast": "VinFast", "Cj Food": "CJ Food", "Fks": "FKS",
                            "Keva & Jotun": "Keva & Jotun", "Jotun, Keva": "Keva & Jotun"
                        })
                        cc_counts = df_cc_clean["Cost Center Clean"].value_counts().reset_index()
                        cc_counts.columns = ["Cost Center", "Jumlah"]
                        fig_cc = px.bar(cc_counts, x="Jumlah", y="Cost Center", orientation="h", title=f"Jumlah Karyawan per Cost Center ({active_period_str})", color="Jumlah", color_continuous_scale="Viridis", text="Jumlah")
                ...
                with c4:
                    if "Site" in df_active_filtered.columns:
                        ...
                        fig_site = px.pie(site_counts, names="Site", values="Jumlah", title=f"Distribusi Lokasi Kerja / Site ({active_period_str})", hole=0.3)
    ...
    if search_query and not df_display.empty:
        ...
        if search_category in df_display.columns:
            df_display = df_display[df_display[search_category].astype(str).str.lower().str.contains(query, na=False)]
        else:
            ...
            with st.form("edit_form"):
                ...
                if btn_save:
                    st.session_state.employees.loc[emp_idx, [
                        ...
                        e_name.strip().title(), e_role.strip(), e_cc.strip(),
                        e_join.strip(), e_end.strip(), e_resign.strip(),
                        e_site.strip(), e_status, str(date.today())
                    ]
